...
# FastPVE Plus — 国内安装脚本（KSpeeder 代理优先）
...
download_link() {
    local name=$1 out=$2
    local ks_url="${KSPEEDER}/${REPO}/releases/download/latest/${name}"
    local fb_url="${FALLBACK_PROXY}${BASE_URL}/${name}"

    if command -v curl &>/dev/null; then
        info "downloading ${name} via KSpeeder..."
        curl -fSL --connect-timeout 3 --max-time 600 --progress-bar -o "$out" "$ks_url" && return 0
        info "retrying via fallback..."
        curl -fSL --connect-timeout 5 --max-time 600 --progress-bar -o "$out" "$fb_url" && return 0
    elif command -v wget &>/dev/null; then
        info "downloading ${name} via KSpeeder..."
        wget -q --timeout=3 --show-progress -O "$out" "$ks_url" && return 0
        info "retrying via fallback..."
